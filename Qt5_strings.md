...
В примерах этой главы, нам не нужно использовать графический пользовательский интерфейс Qt, мы создадим программы для командной строки. Поскольку графический интерфейс включен по умолчанию, мы можем отключить его, добавив объявление QT -= gui в файл проекта.
...
Это инициализация с использованием фигурных скобок.
...
Существует три метода для получения длины строки. Эти методы называются size, count и length. Они возвращают количество символов в указанной строке.
...
Динамическое составление строк позволяет заменить специальные символы на необходимые значения. Мы используем метод arg для интерполяции.
...
~~~
String s3 = "We have %1 lemons and %2 oranges";
int ln = 12;
int on = 4;
...
В примере мы определяем простое предложение. Мы подсчитываем количество цифр, букв, пробелов и знаков пунктуации в этом предложении.
...
Метод rightJustified возвращает строку указанной ширины. Если исходная строка короче, то недостающие символы будут добавлены. Добавлены будут указанные символы из второго параметра. В нашем случае, это пробел.
