...
Виджет QCheckBox имеет два состояния: включено и выключено. Это отметка c текстовой меткой. Если checkbox включен, то в ставится отметка в виде галочки.
...
Виджет QListWidget используется для отображения списка элементов. В нашем примере, мы продемонстрируем как добавлять, переименовывать и удалять элементы из списка.
...
Виджет QListWidget создан и заполнен четырьмя элементами.
...
Метод addItem добавляет элемент в виджет. Метод показывает диалог ввода. Диалог возвращает строковое значение. Мы удаляем возможные пробелы, используя метод simplified. Если возвращаемое значение не пустое, мы добавляем его в виджет, в конец списка. В окне, мы выделяем вставленный элемент с помощью setCurrentRow метод.
...
Переименование элемента состоит из нескольких шагов. Сначала мы должны получить текущий элемент с помощью метода currrentItem. Далее мы получаем текст элемента и строку, где элемент расположен. Текст элемента отображаем в диалоге QInputDialog. Строка, которую возвратил диалог, обрабатывается с помощью метода simpified, для удаления возможных пробелов. Далее, мы удаляем старый метод с помощью метода takeItem, и заменяем его с помощью insertItem метода. Мы удаляем элемент с помощью takeItem метода, потому, что удаленные элементы более не управляются Qt. В конце, с помощью метода setCurrentRow мы выбираем новый элемент.
...
В примере, у нас есть QProgressBar и две кнопки. Одна кнопка запускает таймер, который обновляет progress bar. Другая кнопка останавливает таймер.
